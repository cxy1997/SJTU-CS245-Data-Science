...
# 绘制热力图
def plot_matrix(mat, fname):
    fig = plt.figure()
    plt.clf()
    ax = fig.add_subplot(111)
    ax.set_aspect(1)
    res = ax.imshow(mat, cmap=plt.cm.jet, interpolation='nearest')
    cbar = fig.colorbar(res, ticks=[-1, 0, 1])
    plt.xticks(range(len(headers)), headers_en, rotation='vertical')
    plt.yticks(range(len(headers)), headers_en)
    plt.tight_layout()
...
def plot_corr(df):
    ...
    for column in result.columns:
        if result.dtypes[column] == np.object:
            ...
            print(result[column])
    plot_matrix(result.corr(), 'corr')

# 读取数据文件
headers_en = ['Age', 'Workclass', 'fnlwgt', 'Education', 'Education-Num', 'Martial Status', 'Occupation', 'Relationship', 'Race', 'Sex', 'Capital Gain', 'Capital Loss', 'Hours per week', 'Country', 'Income']
